...
@section('title', 'Gemini Price History')
...
@section('content')
    <div class="container py-4">
                    ...
                    <tbody>
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td>
                                    ${{ number_format($item->last - $average, 2) }}
                                </td>
                                <td>
                                    {{ $item->date }}
                                </td>
        ...
        <div>Price is spot price as quoted by Gemini at approximately the time shown (Eastern Time)</div>
    </div>
@endsection
    ...
    <script>
        ...
        @if($data->isNotEmpty())
            var chartData = {!! json_encode($data->reverse()->values()->map(function($item) {
                return [(float)$item->timestamp, (float)$item->last];
            })->values()->toArray()) !!};
        @else
            var chartData = [];
        @endif
